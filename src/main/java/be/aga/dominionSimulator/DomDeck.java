    ...
    public void gain( DomCard aCard , int aLocation) {
		...
		if (aLocation==DISCARD && aCard.getName()!=DomCardName.Villa){
	    	if (aCard.getName()==DomCardName.Nomad_Camp
             ...
             || (owner.isTravellingFairActive() && aCard.getDiscardPriority(1)>=DomCardName.Copper.getDiscardPriority(1) && owner.getKnownTopCards()<5 && owner.countInDeck(DomCardName.Province)==0)) {
                owner.putOnTopOfDeck(aCard);
            } else {
        ...
        if (aCard.hasCardType(DomCardType.Victory)) {
            ...
            if (owner.getCurrentGame().getBoard().isLandmarkActive(DomCardName.Battlefield) ) {
