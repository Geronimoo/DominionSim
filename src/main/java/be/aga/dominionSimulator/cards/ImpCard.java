...
public class ImpCard extends DomCard {
    ...
    public void play() {
        ...
        if (owner.isHumanOrPossessedByHuman()) {
            ...
            if (getNextActionToPlay() != null) {
               owner.play(owner.getNextActionToPlay());
            }
        }
    }
    ...
    private void handleHuman() {
        ...
        if (theChosenCard != null) {
            owner.play(owner.getCardsFromHand(theChosenCard).get(0));
        }
    }
    ...
    public int getPlayPriority() {
        ...
        for (DomCard card : theActionsToConsider) {
            if (card==this)
                continue;
            if (card.getName()!=DomCardName.Imp && card.wantsToBePlayed() && owner.getCardsFromPlay(owner.getNextActionToPlay().getName()).isEmpty() )
                return 2;
        }
        return super.getPlayPriority();
    }
